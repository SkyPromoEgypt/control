...
var billing = {
	...
	init:function() {
		...
		Core.addEventListener(billing.form._close, 'click', function(event) {
			Alert.openListener(event, 'Are you sure you want to close this booking from your records?', 'confirm', function() { billing.closeBill(); } )
		});
					...
					if(xhr.readyState == 4 && xhr.status == 200) {
						...
						if(responseText) {
							if(responseText == 'done') {
								...
								if(theBill.bkref == null) billing.refreshBill();
							} else {
								billing.form._loading.innerHTML = '';
								Alert.openListener('', 'Booking is already closed', 'alert', '');
								if(theBill.bkref == null) billing.refreshBill();
							}
						}
					} else {
